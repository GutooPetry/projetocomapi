...
def gerar_link_pagamento():
    ...
    if len(cursor.fetchall()) > 0:
        ...
        dados = (result['response']['date_created'], external_reference, "None")
    ...
    if st.session_state.form_to_show == 'aplicar-promo':
        ...
        with st.form('aplicar-promocao', True):
            ...
            if botao_cad:
                ...
                print(lista_codigos)
                if cod_barras in lista_codigos and cod_barras != '' and preco_promo != '':
                    select = f'SELECT id FROM produtos WHERE cod_barras = {cod_barras}'
                    cursor.execute(select)
                    produto_id = cursor.fetchall()[0][0]
...
def registra_status():
    ...
    if consulta_pagamento() == 'approved':
        conn = conexao_db()
        cursor = conn.cursor()
        ...
        sql = """
              ...
              SET p1.status = 'approved'
              WHERE p1.id = p2.max_id;
        """
        cursor.execute(sql)
        conn.commit()

    elif consulta_pagamento() == 'rejected':
        conn = conexao_db()
        cursor = conn.cursor()
...
def debita_estoque():
    ...
    for item in carrinho:
        ...
        sql4 = 'DELETE FROM carrinho WHERE cod_barras = %s;'
        sql5 = 'SET sql_safe_updates = 0;'
        sql6 = 'SET sql_safe_updates = 1;'
