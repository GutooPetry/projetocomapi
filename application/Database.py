...
class Database:
    ...
    def select_vendas(self, data):
        conn = self.conn_sqlalchemy()
        with conn.connect():
            sql = 'SELECT * FROM vendas WHERE data_venda = %s'
            dados = (data,)
            tabela_vendas = pd.read_sql(sql, conn, params=dados)
            return st.dataframe(tabela_vendas, hide_index=True, use_container_width=True)

    def select_produtos_venda(self, venda_id):
        conn = self.conn_sqlalchemy()
        with conn.connect():
            sql = 'SELECT venda_id, cod_barras, qtd_vendida, data_venda FROM produtos_vendidos WHERE venda_id = %s;'
            dados = (venda_id,)
            tabela_produtos = pd.read_sql(sql, conn, params=dados)
            return st.dataframe(tabela_produtos, hide_index=True, use_container_width=True)
